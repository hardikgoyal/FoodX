...
public class DataFetcher {
	...
	// with
																			...
																			// +
																			// yelp_end_url
	private static final String grub_hub_begin_url = "https://www.grubhub.com/search?orderMethod=delivery&locationMode=DELIVERY"; // add
																			private static final String yelp_begin_url = "http://www.yelp.com/search?find_desc=Restaurants&find_loc=";
																																	private static final String yelp_end_url = "&start=0&ytp_st=delivery"; // construct
	...
	public DataFetcher() {
		...
		try {
			...
			while (sc.hasNext()) {
				...
				if (line.length > 5) {
					latlon.put(line[0].substring(1, line[0].length() - 1), new LatLon(
							line[3].substring(1, line[3].length() - 1), line[4].substring(1, line[4].length() - 1)));
				}
			}
	...
	private ArrayList<Restaurant> fetchGrubHub(String zip) {
		...
		String request_url = grub_hub_begin_url + lonAndLat
				+ "facetSet=umami&pageSize=20&hideHateos&variationId=default-impressionScoreBaseBuffed-20160317&countOmittingTimes";
		URL grubHub;
		...
		try {
			...
			while ((inputLine = in.readLine()) != null) {
				...
				if (inputLine.contains("<span class=\"indexed-biz-name\">")) {
					restaurantName = inputLine.substring(inputLine.indexOf("><span >") + "><span >".length(),
							inputLine.indexOf("</span></a>"));

					urlToOrder = "http://www.yelp.com/"
							+ inputLine.substring(inputLine.indexOf("href=\"") + "href=\"".length(),
									inputLine.indexOf("\" data-hovercard-id"));
			...
			for (int i = 0; i < restaurantNames.size(); i++) {
				restaurants.add(
						new Restaurant(restaurantNames.get(i), addreses.get(i), phones.get(i), urlToOrders.get(i)));
			}
